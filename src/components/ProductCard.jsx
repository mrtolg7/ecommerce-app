...
export default function ProductCard({ product }) {
  ...
  return (
    ...
    <div key={product.id} className=" relative group bg-white rounded-xl overflow-hidden transition hover:shadow-lg flex flex-col h-full border border-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:text-white">

      <Link to={`/product/${product.id}`} >
        <div className="h-64 bg-white flex items-center justify-center overflow-hidden relative p-4">
          <img
            src={product.image}
            alt={product.title}
        ...
        <div className="p-4 flex flex-col flex-1">
          <h3 className="text-sm font-medium line-clamp-2 text-gray-700 dark:text-gray-300 min-h-10">
            {product.title}
          </h3>

          <p className="mt-4 mb-2 text-lg font-bold text-gray-900 dark:text-white">
            ${product.price}
          </p>
      ...
      <div className="absolute top-3 right-3">
        <button onClick={() => toggleWishlist(product)}>
          <Heart
            className={`transition duration-200 ${isInWishlist
